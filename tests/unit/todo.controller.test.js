...
// jest.mock is a short hand for ^^^
jest.mock('../../model/todo.model.js')

const todoId = singleTodo._id
...
describe('TodoController.deleteTodo', () => {
	...
	it('should return deleted model and status code of 200', async () => {
...
describe('TodoController.updateTodo', () => {
	...
	it('should update with TodoModel.findByIdAndUpdate', async () => {
		...
		expect(TodoModel.findByIdAndUpdate).toHaveBeenLastCalledWith(todoId, newTodo, {
			new: true,
			useFindAndModify: false
		})
	})
	it('should return a response with updated json data and status code 200', async () => {
		req.params.todoId =  todoId
		req.body = newTodo
	...
	it('should handle errors in updateTodo()', async () => {
		const errorMessage = { message: 'Faild updating todo' }
		const rejectedPromise = Promise.reject(errorMessage)
	...
	it('should return 404 if id doesnt exits', async () => {
...
describe('TodoController.getTodoById', () => {
	...
	it('should call TodoModel.findById with route parameters', async () => {
		req.params.todoId = singleTodo._id
		await TodoController.getTodoById(req, res, next)
		expect(TodoModel.findById).toBeCalledWith(singleTodo._id)
	})
	...
	it('should handle errors in getTodoById()', async () => {
		const errorMessage = { message: 'Faild finding todo' }
		const rejectedPromise = Promise.reject(errorMessage)
...
describe('TodoController.getTodos', () => {
	...
	it('should handle errors in getTodos()', async () => {
		const errorMessage = { message: 'Error getting all todos' }
		const rejectedPromise = Promise.reject(errorMessage)
...
describe('TodoController.createTodo', () => {
	...
	it('should set status code to 201 and send the response', async () => {
		await TodoController.createTodo(req, res, next)
